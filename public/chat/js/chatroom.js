...
(function(){
	...
	var sortableMessages = new Sortables("message-list", {
		...
		onComplete: function(ele){
			sortableMessages.serialize(false, function(element, index){
				message = Messages.get(element.getProperty("id").replace("message-", ""));
				message.save({
					"order": index
				});
			});
	...
	window.AppView = Backbone.View.extend({
		...
		el: $("chatroomapp"),
		statsTemplate: _.template('<% if (total) { %><span class="message-count"><span class="number"><%= total %></span><span class="word"> message <%= remaining == 1 ? "item" : "items" %></span> added.</span><% } %><% if (true) { %><span class="message-clear"><a href="#">Clear <span class="number-done"><%= done %> </span>message <span class="word-done"><%= total == 1 ? "item" : "items" %></span></a></span><% } %>'),
...
//			console.log(socket);
		    if (typeof ipaddr == 'undefined') ipaddr = null;
			...
			socket.on('disconnect', function(){
				// TODO: Display message indicating that the user left the room.
		...
		getGeoLocation: function () {
			if(navigator.geolocation) navigator.geolocation.getCurrentPosition(this.sendGeoLocationToServer, this.geoLocationError);
		...
		appendMessage: function (message){
			...
			if( message=="<p style='color:grey'>Please login in order to chat.</p>"
				|| message=="<p style='color:red'>Disconnected.</p>"
				|| message == "<p style='color:grey'>you have been logged out!</p>")
				if (this.$('#username').getProperty("value") )
					$('password').focus();
				else
					$('username').focus();
		...
		render: function() {
			...
			this.$("#message-stats").set("html",this.statsTemplate({
				done:       done,
